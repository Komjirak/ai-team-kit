...
graph TD
    ...
    subgraph BUILD ["구현 · 아키텍처"]
        APP["📱 APP<br/>RN/Expo 앱 · Extension"]
        FE["🖥️ FE<br/>순수 웹(마케팅 · 웹앱)"]
        BE["🔧 BE<br/>서버 · 데이터 · 결제 · admin"]
        AI["🤖 AI<br/>모델 · 프롬프트 · 평가 · 폴백"]
        TL["🧭 TL<br/>아키텍처 · 기술 표준"]
    end
    subgraph SHIP ["출시"]
...
- **PO는 사람이고 한 명이다.** 팀의 유일한 결정권자이며, 에이전트는 PO를 대체하지 않는다. `komjirak-product-owner`(agent-prompts 51)는 PO가 아니라 **PO의 판정 초안을 만드는 어드바이저**다 — 방향안·판정안까지만 내고, 착수·중단·대외 발행의 확정은 사람 PO가 한다.
- **오케스트레이터는 메인 Claude 세션 자신이다.** 별도 에이전트 파일이 없다 —
  `AGENTS.md` + 이 문서가 곧 오케스트레이터의 하네스다.
- 역할 에이전트 14종은 오케스트레이터가 Agent 도구로 띄우는 서브에이전트다.
  작은 작업이면 오케스트레이터가 해당 역할의 하네스를 직접 읽고 그 모자를 쓰고
  일해도 된다 — **중요한 것은 프로세스(하네스 준수)지 프로세스 격리가 아니다.**
  역할이 14개라고 매번 14번 위임하라는 뜻이 아니다.
...
| 웹 화면 구현 | FE | ORCH | PD·BE | QA |
| AI 기능 연동·평가 | AI | ORCH | DS·BE | QA |
| 아키텍처·기술 표준 | TL | ORCH | APP·BE | PM |
| 지표·실험 설계 | DS | ORCH | PM·PO | 전원 |
...
| APP ↔ FE | APP은 **RN/Expo 앱·익스텐션**, FE는 **순수 웹**(마케팅 페이지·웹앱). 앱에 딸린 웹뷰·공유 코드만 APP이 본다 |
| AI ↔ DS | DS는 **검증**(실험·모델링·인과), AI는 **연동**(프롬프트·파이프라인·평가·비용). 모델이 검증되면 DS → AI로 넘긴다 |
